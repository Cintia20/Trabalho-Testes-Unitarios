...
func _ready():
	...
	if (bird):
		bird.connect("state_changed", Callable(self, "_on_bird_state_changed").bind(), CONNECT_ONE_SHOT)
	pass
...
func go_init_pos():
	...
	init_pos.y = randf_range(0 + OFFSET_Y, get_viewport_rect().size.y - GROUND_HEIGHT - OFFSET_Y)
...
func spawn_pipe():
	...
	new_pipe.connect("tree_exited", Callable(self, "spawn_and_move"))
...
func go_next_pos():
	...
	next_pos.y = randf_range(0 + OFFSET_Y, get_viewport_rect().size.y - GROUND_HEIGHT - OFFSET_Y)
	position = next_pos
	pass
